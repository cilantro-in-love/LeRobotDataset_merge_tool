...
一个用于将多个相同任务 LeRobotDataset 合并为单个统一 LeRobotDataset 的 Python 工具。该工具能够正确处理视频文件、parquet 数据文件和元数据，确保合并后的数据集完整性。

## 功能特性

- ✅ 合并多个 LeRobot 相同任务数据集，同时保留数据结构
- ✅ 自动处理 episode 索引偏移
...
dst_dir = "/path/to/destination/lerobot_dataset"  # 目标数据集路径，源数据追加在此数据集后
...
使用 `data_video_merge.py` 可以分别合并`videos`和`data`文件，需要分别指定源和目标路径，以及源和目标的起始和结束索引。
